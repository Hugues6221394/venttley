-- 0035_friend_profile_heatmap.sql
--
-- Add an activity heatmap to the friends-tier branch of the
-- user_profile_summary RPC. 90 daily buckets (≈13 weeks), each with
-- a count of authored posts + comments. Only emitted when the viewer
-- is `friends` or `self` — strangers still get the stripped tier.
--
-- The data is small (90 rows ≈ a few hundred bytes JSON) and joins
-- cheaply into the existing single-roundtrip payload, so the client
