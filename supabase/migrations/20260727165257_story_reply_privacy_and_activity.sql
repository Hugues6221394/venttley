-- Story privacy and owner activity.
--
-- Reply privacy is enforced in the database so an older or modified client
-- cannot attach a reply to a story after its author disables replies.

ALTER TABLE public.users
  ADD COLUMN IF NOT EXISTS story_replies_enabled BOOLEAN NOT NULL DEFAULT TRUE;

CREATE OR REPLACE FUNCTION public.my_story_replies_enabled()
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SECURITY INVOKER
...
AS $$
  SELECT COALESCE(
    (
      SELECT u.story_replies_enabled
...
CREATE OR REPLACE FUNCTION public.set_my_story_replies_enabled(
  p_enabled BOOLEAN
) RETURNS BOOLEAN
LANGUAGE plpgsql
VOLATILE
...
BEGIN
  ...
  UPDATE public.users AS u
     SET story_replies_enabled = COALESCE(p_enabled, TRUE),
         updated_at = now()
...
REVOKE ALL ON FUNCTION public.my_story_replies_enabled() FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.set_my_story_replies_enabled(BOOLEAN)
  FROM PUBLIC, anon;
...
GRANT EXECUTE ON FUNCTION public.my_story_replies_enabled()
  TO authenticated;
GRANT EXECUTE ON FUNCTION public.set_my_story_replies_enabled(BOOLEAN)
  TO authenticated;
